...
resource "aws_appautoscaling_target" "service" {
  ...
  resource_id        = "service/${local.ecs_cluster_id}/${aws_ecs_service.app[each.key].name}"
...
resource "aws_appautoscaling_policy" "cpu_threshold" {
  ...
  target_tracking_scaling_policy_configuration {
    target_value = var.autoscaling_thresholds.cpu
...
resource "aws_appautoscaling_policy" "memory_threshold" {
  ...
  service_namespace  = aws_appautoscaling_target.service[each.key].service_namespace
  target_tracking_scaling_policy_configuration {
    target_value = var.autoscaling_thresholds.memory
    predefined_metric_specification {
