locals {
  ...
  app_containers_map = {
    ...
    name => merge(container, {
      disk_drive_enabled = contains(["efs", "ebs"], container.disk_drive.type)
    })
  }
  ...
  replacements = {
    ...
    name => {
      ...
      "{cognito_user_pool_arn}"    = contains(local.host_based_user_pools, container.hostname) ? aws_cognito_user_pool.host_based[container.hostname].arn : (local.create_user_pool ? aws_cognito_user_pool.user_pool[0].arn : "")
      "{cognito_user_pool_id}"     = contains(local.host_based_user_pools, container.hostname) ? aws_cognito_user_pool.host_based[container.hostname].id : (local.create_user_pool ? aws_cognito_user_pool.user_pool[0].id : "")
      "{cognito_user_pool_domain}" = contains(local.host_based_user_pools, container.hostname) ? aws_cognito_user_pool.host_based[container.hostname].domain : (local.create_user_pool ? aws_cognito_user_pool.user_pool[0].domain : "")
